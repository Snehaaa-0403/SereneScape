...
function App() {
  ...
  useEffect(() => {
    const fetchSpots = async () => {
      try {
        const response = await fetch('https://serenescape-3.onrender.com//spots')
  ...
  useEffect(()=>{
    const fetchData = async()=>{
      try{
        const response=await fetch('https://serenescape-3.onrender.com//trips');
        const result = await response.json(); 
        if(result.success){
  ...
  const handleLoadTrip = async(tripId)=>{
    try {
      const response = await fetch(`https://serenescape-3.onrender.com//trips/${tripId}`);
      const result = await response.json();
      if (result.success) {
  ...
  const handleRemoveSpot = async(spotId) => {
    ...
    try {
      ...
      const response = await fetch(`https://serenescape-3.onrender.com//trips/${currentTrip._id}/spots/${spotId}`, {
        method: 'DELETE',
      });
  ...
  const handleAddToTrip = async (spotId, day) => {
    ...
    try {
      ...
      const response = await fetch(`https://serenescape-3.onrender.com//trips/${currentTrip._id}/add-spot`, {
        method: 'PUT',
  ...
  const handleAddExpense = async (title, amount) => {
    ...
    try {
      const response = await fetch(`https://serenescape-3.onrender.com//trips/${currentTrip._id}/expense`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
